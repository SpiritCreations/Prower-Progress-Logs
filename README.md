...
This repo is meant to track everything happening on **Prower**, whici is my personal homelab server that runs Proxmox, where I build VMs and containers. The hope is that every change, problem, and fix gets logged here.
...
Many homelab ideas or progress is easily written on disorganized napkins strewn about. I wanted to practice documenting my infrastructure changes the way a real ops team would, that being dated, changelobs, problem/resolution tracking, and postmortems for record keeping that I can learn from.
...
| MCTestBranch-CT-02 | Container | Sanbox Test Environment for CEJ (and any other future servers) |
...
- **`Infrastructure-VM-01.md`** - The core automation node setup, including fail2ban, UFW, and unattended upgrade.
